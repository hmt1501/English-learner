...
- **Vocabulary** uses a traditional type-and-check method (no SRS). `/vocab/study?topic=<id>` runs a per-topic session: phase ① show the English chunk → learner types the Vietnamese meaning → `checkMeaning()` grades it and reveals the answer + example sentence with translation; phase ② VN→EN translation of the session's example sentences, graded by `checkTranslation()`. Grading is offline and lenient (accent-insensitive, token-coverage thresholds) in `src/lib/check.ts` (pure, unit-tested); the learner can always self-override the verdict. Per-word stats (`correct`/`wrong`/`lastSeen`, mastered at 2 correct) persist in IndexedDB via `src/lib/storage.ts` `WordStats`.
